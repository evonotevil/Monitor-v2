...
STATUS_CSS = {
    "已生效":      "background:#DC2626;color:#FFFFFF;",   # 红底白字：法规已生效，最高优先
...
def generate_html(items: List[dict], title: str = "全球互联网合规动态监控报告",
    ...
    for group in _GROUP_ORDER:
        ...
        for item in sorted(
            ...
            enacted_style = " background:#FFF5F5;" if status == "已生效" else ""
            rows_html += (
                f'\n        <tr class="main-row" '
                ...
                f'style="border-left:3px solid {style["border"]};{enacted_style}">'
                ...
                f'<td class="td-summary">{summary_zh}'
                f'{"<div class=rec-note>💡 " + rec_note_esc + "</div>" if rec_note_esc else ""}'
                f'</td>'
                f'</tr>'
            )

    # ── 附录：上月其他合规快讯（纯一行式：日期 · 标题[链接] [地区]）──────
    appendix_html = ""
    if appendix_items:
        ap_lines = ""
        ...
        appendix_html = (
            ...
            f'<div class="appendix-title">📎 上月其他合规快讯（{len(appendix_items)} 条）</div>'
...
<div class="container">
  ...
  <div class="header">
    ...
    <div class="header-brand">
      {logo_html}
      <span class="brand-name">Legal</span>
    </div>
  </div>

  <!-- 月度合规形势综述 -->
  {(
    f'<div class="card exec-summary-card">'
    f'<div class="exec-summary-title">📋 上月合规形势综述</div>'
    f'<div class="exec-summary-body">{html_mod.escape(exec_summary)}</div>'
    f'</div>'
  ...
  <!-- 页脚 -->
  <div class="footer">Legal &nbsp;·&nbsp; 全球互联网合规监控 &nbsp;·&nbsp; 仅供内部参考</div>
...
function sortTable(col) {{
  ...
  groupOrder.forEach(grp => {{
    ...
    grpMainRows.forEach(r => tbody.appendChild(r));
  }});
}}
</script>
</body>
