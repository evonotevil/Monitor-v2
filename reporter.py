...
def _get_logo_html() -> str:
    ...
    if _LOGO_PATH.exists():
        ...
        return f'<img src="data:{mime};base64,{b64}" alt="Bilibili" class="header-logo">'
    return ""
...
def print_table(items: List[dict], max_summary_len: int = 50):
    ...
    print(f"{C.BOLD}  全球互联网合规动态监控报告  |  {datetime.now().strftime('%Y-%m-%d %H:%M')}{C.RESET}")
    print(f"{C.BOLD}{'='*140}{C.RESET}")
    print()
...
def generate_markdown(items: List[dict], title: str = "全球互联网合规动态监控报告") -> str:
...
def generate_html(items: List[dict], title: str = "全球互联网合规动态监控报告",
    ...
    for group in _GROUP_ORDER:
        ...
        _row_idx = [0]  # mutable counter for unique row IDs within this call
        for item in sorted(
            group_items,
            key=lambda x: (
                x.get("date", ""),
            ...
            # 正文与合规提示（可能为空，旧数据无此字段）
            detail_zh_raw       = (item.get("detail_zh")       or "").strip()
            compliance_note_raw = (item.get("compliance_note") or "").strip()
            has_detail = bool(detail_zh_raw or compliance_note_raw)
            ...
            row_id = f"r{id(item)}"
            expand_cls = " has-detail" if has_detail else ""
            expand_hint = '<span class="expand-icon">▶</span>' if has_detail else ''

            rows_html += (
                f'\n        <tr class="main-row{expand_cls}" '
                f'data-row-id="{row_id}" '
                f'data-date="{html_mod.escape(item_date)}" '
                ...
                f'<td class="td-title">'
                f'<span class="td-title-zh">{expand_hint}{zh_headline}</span>'
            ...
            # 正文展开行（隐藏，点击主行显示）
            if has_detail:
                detail_esc = html_mod.escape(detail_zh_raw)
                note_esc   = html_mod.escape(compliance_note_raw)
                detail_body = ""
                if detail_esc:
                    detail_body += f'<p class="detail-text">{detail_esc}</p>'
                if note_esc:
                    detail_body += f'<p class="detail-note">{note_esc}</p>'
                rows_html += (
                    f'\n        <tr class="detail-row" id="detail-{row_id}" style="display:none;"'
                    f' data-group="{html_mod.escape(group)}">'
                    f'<td colspan="6" class="td-detail">'
                    f'<div class="detail-body">{detail_body}</div>'
                    f'</td></tr>'
                )
...
/* ── 正文展开行 ── */
.main-row.has-detail {{ cursor: pointer; }}
.main-row.has-detail:hover .td-title-zh {{ color: #4A4AE0; }}
.expand-icon {{
    font-size: 8px;
    color: #AEAEB2;
    margin-right: 5px;
...
.main-row.expanded .expand-icon {{ transform: rotate(90deg); color: #6E6EF7; }}
.td-detail {{
    padding: 0 24px 14px 40px !important;
    background: #FAFBFF;
    border-left: 3px solid #6E6EF7;
...
<div class="container">
  ...
  <div class="header">
    ...
    <div class="header-brand">
      {logo_html}
      <span class="brand-name">Bilibili Legal</span>
    </div>
  </div>
...
(function() {{
  // 初始化地区分组下拉
  const rows = document.querySelectorAll('#mainTable tbody .main-row');
...
function applyFilters() {{
  ...
  rows.forEach(r => {{
    ...
    // 同步控制展开行
    const detailRow = document.getElementById('detail-' + r.dataset.rowId);
    if (detailRow) detailRow.style.display = (show && r.classList.contains('expanded')) ? '' : 'none';
...
function updateCount(n) {{
  const total = document.querySelectorAll('#mainTable tbody .main-row').length;
...
// 展开/收起正文
document.querySelectorAll('.main-row.has-detail').forEach(row => {{
...
function sortTable(col) {{
  ...
  // 按列排序（忽略分组 header 和展开行，排序后重新按分组插入）
  mainRows.sort((a, b) => {{
  ...
  groupOrder.forEach(grp => {{
    ...
    if (!hdr) return;
    const grpMainRows = mainRows.filter(r => r.dataset.group === grp);
    if (grpMainRows.length === 0) return;
    tbody.appendChild(hdr);
    grpMainRows.forEach(r => {{
      tbody.appendChild(r);
      // 同步插入展开行（如果存在）
      const detailRow = document.getElementById('detail-' + r.dataset.rowId);
